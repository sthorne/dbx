...
func (rs *testRowScanner) ScanRow(rows dbx.Rows) error {
...
func (ers *testErrRowScanner) ScanRow(rows dbx.Rows) error {
...
func TestForEachRow(t *testing.T) {
	...
	pgxtest.RunWithQueryExecModes(ctx, t, defaultConnTestRunner, nil, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		var a, b int
		ct, err := dbx.ForEachRow(rows, []any{&a, &b}, func() error {
...
func ExampleForEachRow() {
	...
	var a, b int
	_, err = dbx.ForEachRow(rows, []any{&a, &b}, func() error {
		fmt.Printf("%v, %v\n", a, b)
		return nil
	})
	if err != nil {
...
func TestCollectRows(t *testing.T) {
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		numbers, err := dbx.CollectRows(rows, func(row dbx.CollectableRow) (int32, error) {
			var n int32
			err := row.Scan(&n)
			return n, err
		})
...
func TestCollectOneRow(t *testing.T) {
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		n, err := dbx.CollectOneRow(rows, func(row dbx.CollectableRow) (int32, error) {
			var n int32
			err := row.Scan(&n)
			return n, err
		})
...
func TestCollectOneRowPrefersPostgreSQLErrorOverErrNoRows(t *testing.T) {
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		name, err = dbx.CollectOneRow(rows, func(row dbx.CollectableRow) (string, error) {
			var n string
			err := row.Scan(&n)
			return n, err
		})
...
func TestCollectExactlyOneRow(t *testing.T) {
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		rows, _ := conn.Query(ctx, `select 42`)
		n, err := dbx.CollectExactlyOneRow(rows, func(row dbx.CollectableRow) (int32, error) {
			var n int32
			err := row.Scan(&n)
			return n, err
		})
...
func TestCollectExactlyOneRowExtraRows(t *testing.T) {
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		assert.ErrorIs(t, err, dbx.ErrTooManyRows)
		assert.Equal(t, int32(0), n)
	})
}
...
func TestRowToAddrOf(t *testing.T) {
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		rows, _ := conn.Query(ctx, `select n from generate_series(0, 99) n`)
		numbers, err := dbx.CollectRows(rows, dbx.RowToAddrOf[int32])
		require.NoError(t, err)
...
func ExampleRowToAddrOf() {
	...
	rows, _ := conn.Query(ctx, `select n from generate_series(1, 5) n`)
	pNumbers, err := dbx.CollectRows(rows, dbx.RowToAddrOf[int32])
	if err != nil {
		fmt.Printf("CollectRows error: %v", err)
		return
	}
...
func TestRowToMap(t *testing.T) {
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		slice, err := dbx.CollectRows(rows, dbx.RowToMap)
		require.NoError(t, err)
...
func TestRowToStructByPos(t *testing.T) {
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		rows, _ := conn.Query(ctx, `select 'Joe' as name, n as age from generate_series(0, 9) n`)
		slice, err := dbx.CollectRows(rows, dbx.RowToStructByPos[person])
		require.NoError(t, err)
...
func TestRowToStructByPosMultipleEmbeddedStruct(t *testing.T) {
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		slice, err := dbx.CollectRows(rows, dbx.RowToStructByPos[meal])
		require.NoError(t, err)
...
func TestRowToStructByPosEmbeddedPointerToStruct(t *testing.T) {
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		rows, _ := conn.Query(ctx, `select 'John' as first_name, 'Smith' as last_name, n as age from generate_series(0, 9) n`)
		_, err := dbx.CollectRows(rows, dbx.RowToStructByPos[person])
...
`)
	...
	products, err := dbx.CollectRows(rows, dbx.RowToStructByPos[product])
	if err != nil {
		fmt.Printf("CollectRows error: %v", err)
		return
	}
...
func TestRowToAddrOfStructPos(t *testing.T) {
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		rows, _ := conn.Query(ctx, `select 'Joe' as name, n as age from generate_series(0, 9) n`)
		slice, err := dbx.CollectRows(rows, dbx.RowToAddrOfStructByPos[person])
		require.NoError(t, err)
...
func TestRowToStructByName(t *testing.T) {
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		slice, err := dbx.CollectRows(rows, dbx.RowToStructByName[person])
		assert.NoError(t, err)
		...
		_, err = dbx.CollectRows(rows, dbx.RowToStructByName[person])
		...
		_, err = dbx.CollectRows(rows, dbx.RowToAddrOfStructByName[person])
...
func TestRowToStructByNameDbTagsCaseInsensitive(t *testing.T) {
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		slice, err := dbx.CollectRows(rows, dbx.RowToStructByName[record])
		require.NoError(t, err)
...
`)
	...
	rows, _ := conn.Query(ctx, "select * from products where price < $1 order by price desc", 12)
	products, err := dbx.CollectRows(rows, dbx.RowToStructByName[product])
	if err != nil {
		fmt.Printf("CollectRows error: %v", err)
		return
	}
...
func TestRowToStructByNameLax(t *testing.T) {
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		slice, err := dbx.CollectRows(rows, dbx.RowToStructByNameLax[person])
		assert.NoError(t, err)
		...
		_, err = dbx.CollectRows(rows, dbx.RowToAddrOfStructByNameLax[person])
		assert.ErrorContains(t, err, "struct doesn't have corresponding row field ignore")
...
func TestRowToStructByNameLaxRowValue(t *testing.T) {
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *dbx.Conn) {
		...
		`)
		slice, err := dbx.CollectRows(rows, dbx.RowToStructByNameLax[UserAPIKey])
...
`)
	...
	rows, _ := conn.Query(ctx, "select * from products where price < $1 order by price desc", 12)
	products, err := dbx.CollectRows(rows, dbx.RowToStructByNameLax[product])
	if err != nil {
		fmt.Printf("CollectRows error: %v", err)
		return
	}
