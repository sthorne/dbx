...
func TestNamedArgsRewriteQuery(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		},
		{
			...
			namedArgs:    dbx.NamedArgs{"extra": int32(1)},
...
func TestStrictNamedArgsRewriteQuery(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		{
			sql:             "@all @matches",
			namedArgs:       dbx.StrictNamedArgs{"all": int32(1), "matches": int32(2)},
			expectedSQL:     "$1 $2",
		...
		{
			sql:             "extra provided argument",
			namedArgs:       dbx.StrictNamedArgs{"extra": int32(1)},
...
func TestStrictStructArgs(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			qr := dbx.StrictStructArgs(tt.input)
			sql, args, err := qr.RewriteQuery(context.Background(), nil, tt.sql, nil)
			if tt.expectError {
				require.Error(t, err)
				return
